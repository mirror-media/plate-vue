...
describe('RecommendList.vue', () => {
  const RecommendListComp = mount(RecommendList)
  it('component RecommendList should have class=\'related-list\'', () => {
    expect(RecommendListComp.hasClass('related-list')).to.equal(true)
  })
  it('component should contain title \'相關文章\'', () => {
    const relatedsTitle = RecommendListComp.first('.related-list__list__title')
  ...
  it('component should render correct relateds', () => {
    RecommendListComp.setProps({
      relateds: relatedsMock
    })
    const relateds = RecommendListComp.find('.related-list__list__item[id^="related-"]')
  ...
  it('component should render correct recommends', () => {
    ...
    const recommends = RecommendListComp.find('.related-list__list__item[id^="recommend-"]')
    const recommendsLen = 10 - relatedsMock.length
    const filteredRecommends = RecommendListComp.vm.filteredRecommends
