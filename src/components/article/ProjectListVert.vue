  ...
  <section class="projectListVert">
    ...
    <div class="projectListVert-list">
      <div class="projectListVert-list__item" :class="[isSquareStyle ? 'square' : '' ]" v-for="item in projects">
        <a :id="`projects-${item.name}-img`" :href="`${siteUrl}/projects/${item.slug}`" :target="[openInNewWindow ? '_blank' : '']" class="projectListVert-list__item--img"><img :src="getImage(item, 'mobile')"></a>
